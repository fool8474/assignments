		...
		void calcHist(cv::InputArray src, int* histogram) {
			...
			for (int y = 0; y < inputMat.rows; y++) {
				for (int x = 0; x < inputMat.cols; x++) {
					...
					/** your code here! **/
					histogram[inputMat.at<uchar>(y, x)] += 1;
		...
		void backprojectHistogram(cv::InputArray src_hsv, cv::InputArray face_hsv, cv::OutputArray dst) {
			...
			for (int y = 0; y < srcMat.rows; y++) {
				for (int x = 0; x < srcMat.cols; x++) {
					...
					/** your code here! **/
					outputProb.at<uchar>(y,x) = UTIL::h_r(model_hist, input_hist, UTIL::quantize(mat_h.at<uchar>(y, x)), UTIL::quantize(mat_s.at<uchar>(y, x))) *255;
		...
		void calcHist_hs(cv::InputArray src_hsv, double histogram[][64]) {
			...
			for (int y = 0; y < hsv.rows; y++) {
				for (int x = 0; x < hsv.cols; x++) {
					...
					/** your code here! **/
					histogram[UTIL::quantize(mat_h.at<uchar>(y, x))][UTIL::quantize(mat_s.at<uchar>(y, x))] += 1;

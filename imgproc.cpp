		...
		void backprojectHistogram(cv::InputArray src_hsv, cv::InputArray face_hsv, cv::OutputArray dst) {
			...
			for (int y = 0; y < srcMat.rows; y++) {
				for (int x = 0; x < srcMat.cols; x++) {
					...
					int a = UTIL::quantize(mat_h.at<uchar>(y, x));
					int b = UTIL::quantize(mat_s.at<uchar>(y, x));

					double n = UTIL::h_r(model_hist, input_hist, a, b);	// 양자화 한 값 넣dj주기.
					outputProb.at<double>(y, x) = n;



					// hint 1 : UTIL::quantize()를 이용해서 srcMat의 값을 양자화합니다. 
